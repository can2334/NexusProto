...
addCommand({ pattern: "^filter ?([\\s\\S]*)", access: "all", desc: "_Add filters that automatically respond to your chats. Supports regexp._", usage: global.handlers[0] + "filter - " + global.handlers[0] + "filter <add || delete || on || off>" }, async (msg, match, sock, rawMessage) => {
    ...
    if (!ifPrivateMessage) {
        ...
        if (!admins.includes(msg.key.participant)) {
            return sock.sendMessage(msg, { translate: true, text: "_You are not an admin in this group!_", edit: msg.key }, { quoted: rawMessage.messages[0] })
        }
    }
    ...
    if (!match[1].trim()) {
        ...
        if (find && find.filters.length > 0) {
            ...
            return await sock.sendMessage(msg, { translate: true, text: "_❌ No filters found._", edit: msg.key }, { quoted: rawMessage.messages[0] });
    ...
    if (match[1].startsWith("delete")) {
        ...
        if (filterIndex !== -1) {
            ...
            return await sock.sendMessage(msg, { translate: true, text: `_✅ Filter deleted successfully._\n_Deleted Filter ::_ \`\`\`${filterToDelete}\`\`\``, edit: msg.key }, { quoted: rawMessage.messages[0] });
        } else {
            return await sock.sendMessage(msg, { translate: true, text: "_❌ No filters found._", edit: msg.key }, { quoted: rawMessage.messages[0] });
        }
    }
    ...
    if (match[1].startsWith("add")) {
        ...
        if (!incoming || !outgoing) {
            return await sock.sendMessage(msg, { translate: true, text: "_❌ Invalid filter format!_\n_Use_ ```.filter add <incoming> <outgoing>```", edit: msg.key }, { quoted: rawMessage.messages[0] });
        }
        const filterIndex = find.filters.findIndex(x => x.incoming === incoming);
        if (filterIndex !== -1) {
            find.filters[filterIndex].outgoing = outgoing;
            return await sock.sendMessage(msg, { translate: true, text: "_✅ Filter updated successfully._", edit: msg.key }, { quoted: rawMessage.messages[0] });

        }
        find.filters.push({ incoming, outgoing });
        return await sock.sendMessage(msg, { translate: true, text: "_✅ Filter added successfully._", edit: msg.key }, { quoted: rawMessage.messages[0] });
    }
    ...
    if (match[1].startsWith("on")) {
        ...
        return await sock.sendMessage(msg, { translate: true, text: "_✅ Filters enabled successfully._", edit: msg.key }, { quoted: rawMessage.messages[0] });
    ...
    if (match[1].startsWith("off")) {
        ...
        return await sock.sendMessage(msg, { translate: true, text: "_✅ Filters disabled successfully._", edit: msg.key }, { quoted: rawMessage.messages[0] });
    }
})
