...
addCommand({ pattern: "^edit ?(.*)", access: "sudo", notAvaliablePersonelChat: true, desc: "_Edit configurations._", usage: global.handlers[0] + "edit <alive || afk || welcome || goodbye>" }, async (msg, match, sock, rawMessage) => {
    ...
    if (!match[1]) {
        return await sock.sendMessage(msg, { text: "_Please enter the configuration to edit._\n\n_Avaliable Configurations ::_ ```alive, welcome, goodbye, afk```", edit: msg.key }, { quoted: rawMessage.messages[0] });
    }

    if (!msg.quotedMessage && !match[1].includes("del")) {
        return await sock.sendMessage(msg, { text: "_Please reply to a message or media to edit._", edit: msg.key }, { quoted: rawMessage.messages[0] });
    ...
    */
    ...
    const updateMessage = async (type, mediaPath, content, configType) => {
        ...
        if (configType === 'alive') {
            ...
            let config = global.database[configKey].find(x => x.chat === msg);
            if (!config) {
                global.database[configKey].push({ chat: msg, type, media: mediaPath ? fs.readFileSync(mediaPath, "base64").toString() : "", content });
            } else {
                config.type = type;
        ...
        let toDelMessage = ""
        if (configType == "welcome") toDelMessage = await global.translateText("\n\n_To delete welcome meessage, use_ ```") + global.handlers[0] + "edit del welcome```";
        if (configType == "goodbye") toDelMessage = await global.translateText("\n\n_To delete goodbye meessage, use_ ```") + global.handlers[0] + "edit del goodbye```";
        if (configType == "afk") toDelMessage = await global.translateText("\n\n_To delete afk meessage, use_ ```") + global.handlers[0] + "edit del afk```";
        return await sock.sendMessage(msg, { text: `_✅ ${configType.charAt(0).toUpperCase() + configType.slice(1)} ${await global.translateText('message updated successfully')}._` + toDelMessage, edit: msg.key }, { quoted: rawMessage.messages[0] });
    ...
    if (configType === "alive" || configType === "welcome" || configType === "goodbye" || configType === "afk") {
        if (imageMessage) {
            ...
            return await sock.sendMessage(msg, { translate: true, text: "_❌ Unsupported message type._", edit: msg.key }, { quoted: rawMessage.messages[0] });
        }
    }

    if (configType == "del welcome" || configType == "del goodbye" || configType == "del afk") {
        let config = configType === "del welcome" ? global.database.welcomeMessage.find(x => x.chat === msg) : configType === "del afk" ? (global.database.afkMessage.active == false ? false : global.database.afkMessage) : global.database.goodbyeMessage.find(x => x.chat === msg);
        if (!config) {
            return await sock.sendMessage(msg, { translate: true, text: "_❌ No " + (configType === "del welcome" ? "welcome" : configType === "del afk" ? "afk" : "goodbye") + " message found._", edit: msg.key }, { quoted: rawMessage.messages[0] });

        } else {
            if (configType === "del welcome") {
                global.database.welcomeMessage = global.database.welcomeMessage.filter(x => x.chat !== msg);
                ...
                return await sock.sendMessage(msg, { translate: true, text: "_✅ AFK message deleted successfully._", edit: msg.key }, { quoted: rawMessage.messages[0] });

            } else {
                global.database.goodbyeMessage = global.database.goodbyeMessage.filter(x => x.chat !== msg);
            }
            return await sock.sendMessage(msg, { translate: true, text: "_✅ " + (configType === "del welcome" ? "Goodbye" : "Welcome") + " message deleted successfully._", edit: msg.key }, { quoted: rawMessage.messages[0] });
        }
    }
})